...
## Mock API Architecture (Static Mode)

For static deployments without a database, VacCOVID uses an embedded mock API system:
...
└────────────┘      └──────────────────┘
       ...
       ▼
┌────────────────────────────────────┐
│     Embedded Data (50+ countries)  │
│  - OWID COVID-19 data (Feb 2023)   │
│  - Vaccine information             │
│  - Archived news                   │
└────────────────────────────────────┘
```
...
|-----------|------|---------|
| Static Server | `server.js` | Express server serving React build and API |
| Mock API | `mockApi.js` | All API endpoints with embedded data |
| React Build | `client/build/` | Pre-compiled React frontend |

### Benefits

- **No Database Required**: All data embedded in JavaScript
- **Minimal Image Size**: ~50MB Docker image
- **Quick Deployment**: Single container, single port
...
See [MOCK_API_GUIDE.md](./MOCK_API_GUIDE.md) for complete endpoint documentation.
