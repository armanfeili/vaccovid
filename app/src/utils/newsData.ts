...
async function _fetchWhoNews() {
    ...
    try {
        ...
        let whoFeed: any = await parser.parseURL("https://www.who.int/rss-feeds/news-english.xml");

        // get the RSS from URL
        let outbreakFeed: any = await parser.parseURL("https://www.who.int/feeds/entity/csr/don/en/rss.xml");
            ...
            await connect.manager.insert(News, [
                {
                    ...
                    content:
                        ...
                        new_News[i].description === "null"
                            ? new_News[i].content
                            : new_News[i].description) &&
                        (new_News[i].content === null || new_News[i].content === "" || new_News[i].content === "null"
    ...
    try {
        if (search_term.length === 0) {
            ...
            for (let i = 0; i < whoNews.length; i++) {
                ...
                whoNews[i].content =
                    pureText + (realLength - validLength < 0 ? "" : `… [+${realLength - validLength} chars]`);
            }
        ...
        if (search_term.length > 0) {
            ...
            uniqueWhoNews = Array.from(new Set(whoNews.map((a) => a.link))).map((link) => {
